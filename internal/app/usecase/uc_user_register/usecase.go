...
func (uc *UseCase) RegisterUser(ctx context.Context, in domain.UserIn) (domain.User, error) {
	...
	if err != nil {
		return domain.User{}, fmt.Errorf("%w: %s", domain.DataValidationError, "birthdate is not valid, the forman should be DD-MM-YYYY")
	}
...
func validate(in domain.UserIn) error {
	if in.Password == "" {
		return fmt.Errorf("%w: %s", domain.DataValidationError, "password is empty")
	}

	if in.FirstName == "" {
		return fmt.Errorf("%w: %s", domain.DataValidationError, "first name is empty")
	}

	if in.Surname == "" {
		return fmt.Errorf("%w: %s", domain.DataValidationError, "surname is empty")
	}

	if in.City == "" {
		return fmt.Errorf("%w: %s", domain.DataValidationError, "city is empty")
	}
